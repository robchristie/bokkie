...
- App-server stdio uses newline-delimited JSON. The client initialises once,
  starts one fresh thread and turn, records their identities before advancing,
  emits schema-valid negative responses to unexpected approval or escalation
  requests, and terminates only on `turn/completed` for the recorded turn.
  Read-only turns use an explicit read-only, network-off sandbox;
  implementation uses workspace-write, network-off access with
