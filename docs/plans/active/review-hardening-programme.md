...
| P4a | Startup-only migration, immutable manifest verification, bounded HTTP database execution, consistent read snapshots and read-only doctor | P3 schema | Terminal candidate on `codex/p4a-storage-integrity-doctor`; locked local checks pass; repair/adoption remains unauthorised |
